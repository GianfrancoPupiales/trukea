...
Plataforma web de intercambio y trueque de productos para estudiantes de la Escuela Politécnica Nacional (EPN). Los estudiantes pueden publicar productos, hacer ofertas de intercambio, chatear en tiempo real y construir reputación a través de intercambios exitosos.
...
- **Gestión de Usuarios**
  ...
  - Edición de perfil (nombre, email, teléfono, foto, contraseña)

- **Catálogo de Productos**
  - Publicación de productos con foto, descripción, categoría y estado
  - Preferencias de categorías aceptadas para intercambio (hasta 3)
  - Búsqueda por título y filtrado por categorías
...
- **Sistema de Ofertas**
  ...
  - Confirmación de entrega física por ambas partes
  - Cancelación automática de ofertas conflictivas al aceptar una oferta
  - Historial completo de ofertas enviadas y recibidas
  - Calificación mutua (1-5 estrellas) después de completar intercambios
  - Bloqueo optimista (@Version) para prevenir condiciones de carrera
...
- **Arquitectura Monolítica Modular**: Organizada por dominios (auth, student, product, offer, chat, reputation, profile, trade)
- **Persistencia con JPA/Hibernate**: Gestión automática de esquema
- **Seguridad robusta**: Autenticación basada en formularios, protección CSRF, sesiones HTTP (30 minutos)
...
- **Logging detallado**: Trazabilidad completa de operaciones (nivel DEBUG para SQL)
- **Optimización de rendimiento**: Lazy loading controlado, índices en BD, prevención de LazyInitializationException
...
- **RabbitMQ**: Message broker para clustering (opcional)
...
```bash
git clone https://github.com/GianfrancoPupiales/trukea.git
cd trukea
```
...
### 3. Instalar RabbitMQ con Docker (Opcional - Solo para Clustering)

**NOTA:** RabbitMQ solo es necesario si deseas ejecutar múltiples instancias de la aplicación (modo clustering). Para desarrollo con una sola instancia, puedes saltar este paso.
...
El archivo JAR se generará en: `target/trukea-0.0.1-SNAPSHOT.jar`

### Ejecutar el JAR

```bash
java -jar target/trukea-0.0.1-SNAPSHOT.jar
```
...
# JAR
java -jar target/trukea-0.0.1-SNAPSHOT.jar --server.port=8081
```
...
Puedes usar el siguiente script de CMD para ejecutar 3 instancias:

**Guardar como `run-cluster.cmd`:**

```cmd
REM Terminal 1 - Puerto 8080
start cmd /k mvnw.cmd spring-boot:run

REM Terminal 2 - Puerto 8081
start cmd /k mvnw.cmd spring-boot:run -Dspring-boot.run.arguments=--server.port=8081

REM Terminal 3 - Puerto 8082
...
**O ejecutar manualmente en diferentes terminales:**
...
4. Creación de registro de reputación asociado (1:1)
...
2. Spring Security valida credenciales con `AuthUserDetailsService`
3. Creación de sesión HTTP (30 minutos de timeout)
4. `LoginSuccessHandler` guarda objeto `AuthUser` en sesión
5. Redirección al catálogo de productos
...
3. Token guardado en tabla `password_reset_tokens` con expiración de 15 minutos
4. Código enviado al correo del usuario vía Mailtrap (`EmailService`)
...
**Características:**
- Limpieza automática de tokens expirados (diaria a las 2 AM con `@Scheduled`)
...
**Proceso:**
1. Usuario completa formulario con título, descripción, categoría, estado y foto
2. Foto guardada en `C:/trukea/uploads/products/` vía `FileStorageService`
3. Producto creado con estado `isAvailable=true`
4. Usuario puede seleccionar hasta 3 preferencias de categorías para intercambio
5. Solo el nombre del archivo se guarda en BD (timestamp_nombreoriginal)
...
- Visualización de foto, título, categoría, estado y dueño
...
- Sistema valida:
  ...
  - Productos ofrecidos pertenecen al usuario

**6.2. Revisión por el Dueño**
- Dueño ve ofertas en `/offers/inbox`
- Opciones: Aceptar o Rechazar
- Vista de productos ofrecidos con fotos y descripciones

**6.3. Aceptación (Estado: ACCEPTED)**
- Sistema ejecuta transacción atómica:
  - Marca todos los productos involucrados como `isAvailable=false`
  - Cancela automáticamente otras ofertas pendientes que involucren esos productos
  - Crea conversación de chat entre ambos estudiantes (`ChatService.createConversationFromOffer`)
  - Usa bloqueo optimista (@Version) para evitar condiciones de carrera
...
**Proceso:**
- Ambos usuarios deben confirmar si el intercambio físico se realizó exitosamente
- **Si ambos confirman éxito:**
  - Estado cambia a COMPLETED
  - Productos permanecen no disponibles
  - Ambos usuarios califican al otro (1-5 estrellas)
  - Reputación actualizada con calificaciones
- **Si alguno reporta falla:**
  - Productos se reactivan (`isAvailable=true`)
  - Reputación del responsable penalizada (score = 1)
...
- Cálculo automático de promedio (totalScore / totalVotes)
- Promedio visible en perfil público
- Bloqueo optimista para evitar inconsistencias
...
- Servidor: Spring WebSocket + RabbitMQ STOMP Relay (opcional)

**Creación de Conversaciones:**
- Automática al aceptar una oferta (`ChatService.createConversationFromOffer`)
- Asociada a dos participantes (student1, student2)
- Relación Many-to-Many con ofertas (una conversación puede tener múltiples ofertas)
...
- Panel derecho: Chat activo
  - Header con info del contacto y reputación
...
- Almacenamiento: Mensaje guardado en BD (tabla `chat_messages`) vía `ChatService.sendMessage`
...
- Marcado como leído al abrir conversación (`ChatService.markMessagesAsRead`)
...
- Endpoint de envío: `/app/chat/{conversationId}` (manejado por `ChatMessageController`)
...
- `totalVotes`: Número total de calificaciones recibidas
- `totalScore`: Suma acumulada de todas las calificaciones
- `averageScore`: Promedio de calificaciones (totalScore / totalVotes)
- Bloqueo optimista con @Version para actualizaciones concurrentes
- Actualización automática al recibir nueva calificación

**Visualización:**
- Perfil público: `/profile/public/{id}`
...
- GET `/profile/public/{id}` - Ver perfil público de otro usuario
...
- Vista pública muestra productos del usuario y reputación
- Conserva foto existente si no se actualiza

### 10. Historial de Intercambios

**Ruta:** `/trades/history`

**Características:**
- Lista de todos los intercambios completados del usuario
- Información de productos intercambiados
- Calificaciones recibidas y dadas
...
│   │   │   │   └── web/                 # AuthWebController, LoginSuccessHandler
│   │   │   │
│   │   │   ├── student/                 # Estudiantes
...
│   │   │   │   ├── dto/                 # ProfileUpdateForm
│   │   │   │   └── web/                 # ProfileController
│   │   │   │
│   │   │   ├── trade/                   # Historial de intercambios
│   │   │   │   └── web/                 # TradeHistoryController
│   │   │   │
...
│   │   │   │   └── web/                 # HomeController, GlobalControllerAdvice
│   │   │   │
│   │   │   └── TrukeaApplication.java   # Main class con @EnableScheduling
│   │   │
│   │   └── resources/
│   │       ├── templates/               # Plantillas Thymeleaf
│   │       │   ├── chat/                # Chat UI (chat.html, mailbox.html)
│   │       │   ├── offers/              # Vistas de ofertas
│   │       │   ├── products/            # Catálogo y gestión de productos
│   │       │   ├── profile/             # Perfiles (my_profile.html, public_profile.html)
│   │       │   ├── trade/               # Historial de intercambios
│   │       │   ├── fragments/           # Componentes reutilizables (navbar, alert, etc.)
...
│   │       │
│   │       ├── application.properties   # Configuración simple
...
│ isAvailable     │
│ publicationDate │
│ preferences     │  (hasta 3 categorías)
│ student_id (FK) │
└────────┬────────┘
         │
         │ N:1
...
│ offererDeliveryConfirmed (boolean)          │
│ ownerDeliverySuccess (boolean)              │
│ offererDeliverySuccess (boolean)            │
│ ownerRating (1-5)                           │
│ offererRating (1-5)                         │
...
- PK: `id_student`
- UK: `email`
- Columnas: email, name, password (BCrypt), ci, phone, address, photo, unique_code

**reputation**
- PK: `id_reputation`
- FK: `student_id` (1:1 con Student)
- Columnas: total_votes, total_score, average_score, version

**products**
- PK: `id_product`
- FK: `student_id` (N:1 con Student)
- Columnas: title, description, category, state, photo, is_available, publication_date, preferences

**offers**
- PK: `id_offer`
- FK: `product_to_offer_id` (producto objetivo)
- FK: `student_who_offered_id` (quien hace la oferta)
- Columnas: status, created_at, accepted_at, completed_at, owner_delivery_confirmed, offerer_delivery_confirmed, owner_delivery_success, offerer_delivery_success, owner_rating, offerer_rating, version
...
2. Verificar que RabbitMQ está corriendo (solo en modo relay)
...
**Problema: Error al compilar SecurityConfig**

```
constructor DaoAuthenticationProvider in class ... cannot be applied to given types
```

**Causa:** Spring Security 7.x (incluido en Spring Boot 4.0.0) cambió la API de `DaoAuthenticationProvider`.

**Solución:** El constructor ahora requiere `UserDetailsService` como parámetro:
...
**Problema: OptimisticLockException al actualizar ofertas o reputación**

```
org.springframework.orm.ObjectOptimisticLockingFailureException
```

**Causa:** Dos usuarios intentan actualizar la misma entidad simultáneamente.

**Solución:** El sistema maneja esto automáticamente con `@Version`. Reintenta la operación o informa al usuario.
...
**Repositorio:** https://github.com/GianfrancoPupiales/trukea
